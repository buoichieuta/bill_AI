...
def process_invoice_stream(image, api_key, model_choice, enhance, use_local_model, fast_mode, progress=gr.Progress(track_tqdm=False)):
    overlay_img = None
    ...
    add_status_line(status_lines, f"Khởi tạo extractor: {model_choice}", started_at)
    progress(0.08, desc="Khởi tạo model")
    yield snapshot("running")
    extractor, err = get_shared_extractor(api_key, model_choice)
    if err:
    ...
    try:
        ...
        yield snapshot("running")
        preprocessed_image, preprocess_meta = preprocess_receipt_image(image, extra_enhance=bool(enhance))
        ...
        if use_local_model:
            add_status_line(status_lines, "Đang chạy best.pt để tìm vùng hóa đơn + field boxes", started_at)
            ...
            if detector:
                ...
                field_boxes = list((detector_meta or {}).get("field_boxes", []))
                if detector_meta:
                    det_kind = detector_meta.get("detector_kind", "?")
                    det_count = detector_meta.get("field_box_count", detector_meta.get("detector_boxes_used", 0))
                    add_status_line(
                        status_lines,
                        f"best.pt hoạt động: kind={det_kind}, boxes={det_count}, bbox={detector_meta.get('detector_bbox')}",
                        started_at,
                    )
                else:
            ...
            preprocess_mode = "preprocess->detect->filter->gemini" if fast_mode else "preprocess->detect->filter->ocr->gemini"
            yield snapshot("running")
        else:
            preprocess_mode = "preprocess->filter->gemini" if fast_mode else "preprocess->filter->ocr->gemini"
            add_status_line(status_lines, "Bỏ qua best.pt theo cấu hình hiện tại", started_at)
            yield snapshot("running")

        add_status_line(status_lines, "Filter ảnh trước OCR/Gemini", started_at)
        progress(0.40, desc="Filter ảnh")
        yield snapshot("running")
        image_for_extract = filter_for_gemini(image_for_extract, extra_enhance=bool(enhance))
        ...
        processed_image_path = save_intermediate_image(
            image_for_extract,
            BASE_DIR,
            prefix="detected_filtered" if use_local_model else "filtered",
        )
        ...
        custom_prompt = None
        if fast_mode:
            add_status_line(status_lines, "Chế độ nhanh: gửi ảnh trực tiếp cho Gemini", started_at)
            progress(0.64, desc="Fast mode")
            yield snapshot("running")
        else:
            try:
                add_status_line(status_lines, "Gemini pass 1: OCR lines + bbox", started_at)
                progress(0.58, desc="OCR lines")
                yield snapshot("running")
                ocr_lines, ocr_meta = extract_ocr_lines(extractor, image_for_extract, field_boxes)
                rule_draft = build_rule_draft(ocr_lines, field_boxes)
                custom_prompt = build_extraction_prompt(
                    image_for_extract,
                    ocr_lines,
                    rule_draft,
                    BASE_DIR / "prompts" / "invoice_extraction_prompt.txt",
                    field_boxes,
                )
                add_status_line(status_lines, f"OCR xong: {len(ocr_lines)} dòng", started_at)
                yield snapshot("running")
            except Exception as ocr_exc:
                ocr_err = f"OCR lines fallback: {ocr_exc}"
                logger.warning(ocr_err)
                add_status_line(status_lines, ocr_err, started_at)
                yield snapshot("running")

        add_status_line(status_lines, "Gemini trích xuất JSON hóa đơn", started_at)
        progress(0.78, desc="Trích xuất JSON")
        yield snapshot("running")
        if custom_prompt:
            result = extractor.extract(image_for_extract, prompt=custom_prompt)
        else:
        ...
        if use_local_model:
            ...
            result_meta["detector_error"] = detector_err
        if ocr_meta:
            result_meta.update(ocr_meta)
        if ocr_err:
            result_meta["ocr_error"] = ocr_err
...
def build_ui():
    with gr.Blocks(title="Invoice AI 🧾") as demo:
        ...
        # Config
        with gr.Accordion("⚙️ Cấu hình API", open=False):
            with gr.Row():
                api_key_box = gr.Textbox(
                    label="Gemini API Key (để trống nếu đã có trong .env)",
                    placeholder="AIzaSy...",
                    type="password", scale=3
                )
                model_dd = gr.Dropdown(
                    label="Model",
                    choices=["gemini-flash-latest", "gemini-2.0-flash", "gemini-2.0-flash-lite", "gemini-2.5-pro-exp-03-25", "gemini-2.0-pro-exp-02-05"],
                    value="gemini-flash-latest", scale=1
                )
            enhance_cb = gr.Checkbox(label="🔆 Tăng cường ảnh (contrast + sharpness)", value=False)
            detector_cb = gr.Checkbox(
                label="🎯 Dùng best.pt để cắt vùng hóa đơn trước khi trích xuất",
                value=(BASE_DIR / "best.pt").exists()
            )
            fast_mode_cb = gr.Checkbox(
                label="⚡ Chế độ nhanh: bỏ pass OCR lines + bbox để giảm thời gian",
                value=True
            )
        ...
        btn_submit.click(
            ...
            inputs=[img_input, api_key_box, model_dd, enhance_cb, detector_cb, fast_mode_cb],
